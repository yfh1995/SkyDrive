...
class ResetController extends Controller{
    ...
    public function postreset(Request $request,$id){
        ...
        if($newpassword!=$repeatnewpassword){
            return Redirect::route('/reset')->withInput()->withErrors('两次输入的密码不一样，请重试！');
        }
            ...
            if(DB::table('users')->where('id',$id)->update(array('password'=>hash::make($newpassword)))){
                ...
                return Redirect::route('/reset')->withInput()->withErrors('旧密码输入错误！请重试！');
            }
        }
        else{
            return Redirect::route('/reset')->withInput()->withErrors('更新失败！');
        }
    }
